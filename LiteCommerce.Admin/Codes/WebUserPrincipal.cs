...
namespace LiteCommerce.Admin
{
      ...
      {
            //IPrincipal  => PHAN QUYEN NGUOI DUNG 
            ...
            /// <summary>
            /// Kiểm tra Role của người dùng (THAY DA LAM LAI)
            /// </summary>
            /// <param name="role"></param>
            /// <returns></returns>
            public bool IsInRole(string role)
            {
                 // Từ danh sách các gr name phân cách nhau bởi ","
                 string[] groups = userData.GroupName.Split(',');
                 //so sanh ( 1 nguoi 1 gr) neu nhieu thi dung mang
                 // truyeen vao [Authorization(Roles ="employee")] trong cac action tuong ung
                  if (groups.Contains(role)) //(role.Equals(userData.GroupName))
                  {
                        return true;
                  }
